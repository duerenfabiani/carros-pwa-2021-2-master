...
function Carro() {
    ...
    const [listaObjetos, setListaObjetos] = useState(
        localStorage.getItem('CARROSPWA-AULA/listaobjetos')
        ? JSON.parse(localStorage.getItem('CARROSPWA-AULA/listaobjetos')) : []);

    const [alerta, setAlerta] = useState( { status: "", message: "" });

    const [objeto, setObjeto] = useState({id: 0, modelo: "", fabricante: "", ano:""});
    ...
    const acaoCadastrar = e => {
        ...
        if (editar) {
            ...
            setAlerta({ status: "success", message: "Carro editado com sucesso" });
        } else { // novo carro
            if (objeto.id === 0) {
                var idautal = localStorage.getItem('CARROSPWA-AULA/sequenciaid');
                if (idautal === null){
                    idautal = 0;
                }
                ...
                localStorage.setItem('CARROSPWA-AULA/sequenciaid', novoid);
                setListaObjetos([...listaObjetos, objeto]);
                setAlerta({ status: "success", message: "Carro criado com sucesso" });
            }
        }
    };  
    ...
    const acaoRemover = objeto => {
        if (window.confirm("Remover este carro?")){
            ...
            setAlerta({ status: "success", message: "Carro removido com sucesso!" })     
        }
    }

    useEffect( () => {
        localStorage.setItem('CARROSPWA-AULA/listaobjetos', JSON.stringify(listaObjetos));
    },[listaObjetos]);
